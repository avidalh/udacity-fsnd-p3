...
def showItems():
    ...
    latestCategories = session.query(Categories).\
        order_by(Categories.id.desc()).limit(5).all()

    latestItems = session.query(Items).\
        order_by(Items.id.desc()).limit(7).all()
...
def showCategories():
    ...
    for category in categories:
        ...
        category.creator = session.query(Users).\
            filter_by(id=category.user_id).one()
...
def newCategory():
    ...
    if request.method == 'POST':
        if request.form['btn'] == 'save':
            ...
            flash('New category %s successfully added!' %
                  newCategory.vehicle_type)
...
# End point JSON / XML
@app.route('/itemCatalog/category/<int:category_id>/item/<int:item_id>/endPoint/<endPoint>/')  # noqa
...
def newItem(category_id):
    ...
    if request.method == 'POST':
        if request.form['btn'] == 'save':
            ...
            session.add(newItem)
            session.commit()
            flash('New item %s %s successfully added!' %
                  (newItem.make, newItem.model))
...
def editItem(category_id, item_id):
    ...
    if request.method == 'POST':
        if request.form['btn'] == 'save':
            ...
            session.commit()
            flash('Changes in %s %s saved!' %
                  (editedItem.make, editedItem.model))
...
# Delete an item
@app.route('/itemCatalog/category/<int:category_id>/items/<int:item_id>/delete',  # noqa
           methods=['GET', 'POST'])
...
# Returns category.vehicle_type for category_id
...
# Returns user_id for user.email
def getUserID(email):
    try:
...
# ends the authentication code.
#
# ______________________oauth_______________________


# Setting the debug flag accessible from templates
...
def make_session_permanent():
    if 'timeStamp' in login_session:
        if time.time() - login_session['timeStamp'] > 300:  # 5 minutes
            ...
            return redirect(url_for('showItems'))
        # if activity in session: refresh timer
        login_session['timeStamp'] = time.time()
